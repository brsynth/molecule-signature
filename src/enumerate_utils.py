...
from src.signature import SignatureNeighbor
from src.signature_alphabet import SignatureSortedArray
...
def ReducedFingerprint(smi, radius=2, useFeatures=False):
    # ARGUMENTS:
    # smi: a SMILES string
    # radius: radius of the fingerprint
    # useFeatures: a boolean. False for ECFP, True for FCFP
    # RETURNS:
    # Reduced ECFP
...
def AtomSignatureRootNeighbors(asig):
    # Local function
    # ARGUMENTS:
    # asig: atom signature 
    # RETURNS:
...
def BondSignatureOccurence(bsig, asig):
    # Local function
    # ARGUMENTS:
    ...
    btype = bsig.split('|')[1]
    asig0, asign = AtomSignatureRootNeighbors(asig)
    asig0 = SignatureNeighbor(asig0)
    asign = [SignatureNeighbor(s) for s in asign]
...
def ConstraintMatrix(AS, BS, Deg, verbose=False):
    ...
    for i in range(BS.shape[0]):
        for j in range(AS.shape[0]):
            asj, bs1, bs2, occ1, occ2 = BondSignatureOccurence(BS[i], AS[j])
            if bs1 == asj:
                C[i,j] = occ2
...
def BondMatrices(AS, NAS, Deg, unique=True, verbose=False):
    ...
    ABS, BBS = [], []
    for i in range(N):
        asig0, asign = AtomSignatureRootNeighbors(AS[i])
        for k in range(K):
            if k < len(asign):
...
def GetConstraintMatrices(sig, unique=True, verbose=False):
    # Callable function
    # ARGUMENTS:
...
def UpdateConstraintMatrices(AS, IDX, MIN, MAX, Deg, verbose=False):
    ...
    return AS, IDX, MIN, MAX, Deg, C, AS, BS
