...
from src.utils import VectorToDic, DicToVector
from src.signature import GetMoleculeSignature
...
class SignatureAlphabet:
    ...
    def fill(self, Smiles, verbose=False):
        ...
        for i in range(len(Smiles)):  
            ...
            if i % 1000 == 0:
                print(f'... processing alphabet iteration: {i} size: {len(list(Dict))} time: {(time.time()-start_time)}')
                start_time = time.time()
            if '*' in Smiles[i]: # no wild card allowed
                continue
            signature, _, _ = SignatureFromSmiles(Smiles[i], self, 
                                                  neighbor=True, verbose=verbose)
            if len(signature) == 0:
                if verbose:
...
def SignatureFromSmiles(smiles, Alphabet, 
                        neighbor=False, string=True, verbose=False):
    # Callable function
    ...
    # Alphabet: cf. SignatureAlphabet object
    # neighbor: return a signature neighbor when True 
    ...
    from .signature import SanitizeMolecule
    from .signature import GetMoleculeSignature
